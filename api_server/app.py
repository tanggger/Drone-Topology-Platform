...
def run_simulation_task(task_id, config):
    ...
    try:
        ...
        def safe_get_int(key, default_val):
            v = config.get(key)
        ...
        leader_node_id = safe_get_int("leaderNodeId", 0)
        backup_leader_list = config.get("backupLeaderList", "")
        distributed_hop_limit = safe_get_int("distributedHopLimit", 1)
        cooperative_failure_type = config.get("cooperativeFailureType", "node_failure")
        failure_target_id = safe_get_int("failureTargetId", -1)
        failure_start_time = safe_get_float("failureStartTime", -1.0)
        failure_duration = safe_get_float("failureDuration", -1.0)
        recovery_policy = config.get("recoveryPolicy", "global_recovery")
        recovery_objective = config.get("recoveryObjective", "connectivity")
        recovery_cooldown = safe_get_float("recoveryCooldown", 1.0)
        ...
        manual_strike_target = safe_get_int("manualStrikeTarget", -1)
        attack_execute_time = safe_get_float("attackExecuteTime", -1.0)
        attack_evaluation_duration = safe_get_float("attackEvaluationDuration", 12.0)
        attack_neighborhood_hop = safe_get_int("attackNeighborhoodHop", 1)
        urban_altitude_penalty_db_low = safe_get_float("urbanAltitudePenaltyDbLow", -1.0)
        urban_altitude_gain_db_high = safe_get_float("urbanAltitudeGainDbHigh", -1.0)
        urban_street_canyon_factor = safe_get_float("urbanStreetCanyonFactor", -1.0)
        lake_volatility_jitter_db = safe_get_float("lakeVolatilityJitterDb", -1.0)
        lake_deep_fade_probability = safe_get_float("lakeDeepFadeProbability", -1.0)
        lake_deep_fade_max_db = safe_get_float("lakeDeepFadeMaxDb", -1.0)
        lake_reflection_delay_jitter_ms = safe_get_float("lakeReflectionDelayJitterMs", -1.0)
        carrier_frequency_ghz = safe_get_float("carrierFrequencyGHz", -1.0)
        channel_bandwidth_mhz = safe_get_float("channelBandwidthMHz", -1.0)
        polarization_mode = config.get("polarizationMode", "")
        reroute_pressure_factor = safe_get_float("reroutePressureFactor", -1.0)
        control_message_urgency_factor = safe_get_float("controlMessageUrgencyFactor", -1.0)
        relay_instability_factor = safe_get_float("relayInstabilityFactor", -1.0)
        formation_reconfig_penalty = safe_get_float("formationReconfigPenalty", -1.0)
