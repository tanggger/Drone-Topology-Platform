...
输出位置：
- 图片保存在 bench_pic/link_survival/ 文件夹
"""
...
class LinkSurvivalVisualizer:
    def __init__(self, benchmark_dir):
        ...
        # 输出目录设置为 bench_pic/link_survival/
        self.output_dir = os.path.join(benchmark_dir, 'bench_pic', 'link_survival')
    ...
    def generate_all_visualizations(self):
        ...
        for formation in self.formations:
            for difficulty in self.difficulties:
                dataset_name = f"{formation}_{difficulty}"
                # 由于采集时命名有误：在绘图阶段交换 Easy 与 Hard 的数据来源
                ...
                topology_file = os.path.join(source_dataset_dir, 'topology-changes.txt')
                
                if not os.path.exists(topology_file):
                    print(f"⚠ Skipping {dataset_name}: file not found")
                    continue
                
                print(f"\n📊 Processing: {dataset_name} (using data from {formation}_{source_difficulty})")
...
def main():
    ...
    # 创建可视化器
    visualizer = LinkSurvivalVisualizer(benchmark_dir)
